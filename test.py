...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("""UPDATE record SET amount = {} WHERE id = :id AND stock = :stock""".format(new_amount), id=session["user_id"], stock=request.form.get("symbol"))
        db.execute("""UPDATE record SET value = {} WHERE id = :id AND stock = :stock""".format(new_value), id=session["user_id"], stock=request.form.get("symbol"))
...
def sell():
    ...
    if request.method == "POST":
        ...
        if not previous_shares:
            ...
            #             price=symb["price"],
            #             value = symb["price"] * shares)
            return apology("Can't sell something you don't have")
